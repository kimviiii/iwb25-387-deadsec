...
import { MatchResult, Volunteer, Event } from "@/types";
...
export const MatchList = ({ volunteers }: MatchListProps) => {
  ...
  // Get full event details for a match
  const getEventForMatch = (match: MatchResult): Event | undefined => {
    return events.find(event => event.id === match.eventId);
  };
                    ...
                    <SelectItem key={volunteer.id} value={volunteer.id}>
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {volunteer.location} • {volunteer.skills.length} skills
                        </span>
                      </div>
                    </SelectItem>
                  ))}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {matches.map((match, index) => {
              ...
              return (
                <Card key={match.eventId} className="shadow-soft border-border/50">
